...
# Moving average buffer for smoothing the mid point locations.
...
def calculate_xmid_xlow(vertical_lines, IMAGE_HEIGHT):
    x_mid_list = []
    x_low_list = []
    ...
    if len(x_mid_list) < 2 or len(x_low_list) < 2 :
    ...
    x_mid_list.sort()
    x_low_list.sort()
    x_mid = np.mean([x_mid_list[0], x_mid_list[1]])
    x_low = np.mean([x_low_list[0], x_low_list[1]])
    """
...
def main():
    ...
    while True:
        ...
        if x_mid is None or x_low is None or x_mid_diff is None:
            continue
        ...
        dev_px, angle_rad = calculate_dev_angle(x_mid_smooth, x_low_smooth, x_mid_diff_smooth, IMAGE_WIDTH, IMAGE_HEIGHT)
        ...
        dev_cm = dev_px * (GRID_SIZE_CM / x_mid_diff_smooth)
